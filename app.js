...
function toggleSelectRow(e, id) {
  if (selectMode && e.target.tagName !== 'BUTTON' && e.target.tagName !== 'INPUT') {
    toggleSelect(id);
    ...
    <tr class="done-row ${selectedIds.has(c.id) ? 'selected' : ''}" id="ci-${c.id}">
      ...
      <td>
        <div class="row-actions">
          <button class="qbtn-row qbtn-row-edit" onclick="openDrawer('${c.id}')" title="Edit">✏️ Edit</button>
          <button class="qbtn-row qbtn-row-done" onclick="quickStatusRow('${c.id}','DONE')" title="Done">✓ Done</button>
          <button class="qbtn-row qbtn-row-pend" onclick="quickStatusRow('${c.id}','PEND/KIV')" title="Pend">⏳ Pend/KIV</button>
...
function generateTicketId() {
  ...
  const todayTickets = complaints.filter(c => c.id && c.id.startsWith(prefix));
  let maxSeq = 0;
...
function handleFiles(e) {
  ...
  for (const f of newFiles) {
    if (f.size > maxBytes) { hasOversize = true; }
    else { validFiles.push(f); }
  }
  if (hasOversize) showToast('limit to 2 mb file', 'error');
...
function renderAttachments() {
  ...
  if (attachmentFiles.length === 0) {
    ...
    preview.innerHTML = attachmentFiles.map((f, i) => {
      ...
      return `<img class="attachment-thumb" src="${url}" title="${esc(f.name)}" alt="${esc(f.name)}" onclick="openImageModal('${url}')" style="cursor:zoom-in;" />`;
    }).join('');
  }
}
